 ...
 */
public class ItemFavoriteAsyncTask extends AsyncTask<String, integer, String> {
    ...
    public ItemFavoriteAsyncTask(Activity activity, ArrayList<ItemData> itemList, int position) {
        this.activity = activity;
        this.itemList = itemList;
        this.position = position;
    }

    public ItemFavoriteAsyncTask(Activity activity, ArrayList<ItemAndCommentData> itemAndCommentDataList) {
        this.activity = activity;
...
//        System.out.println(urls[0]);
    ...
    protected void onPostExecute(String str) {
        try{
            ...
            itemList.get(position).setMyFavorite(root.getInt("result"));

            // 목록 페이지의 정보과 업데이트되도록 목록 리스트의 아이템 변경
            if (activity instanceof ListActivityShowingLikesAndComments)
                ((ListActivityShowingLikesAndComments)activity).updateAnItem(position);

            // 상세 페이지의 정보과 업데이트되도록 0번 아이템(아이템 상세정보) 변경
            if (activity instanceof DetailActivityWithCommentAndReport) {
                itemAndCommentDataList.set(0, itemList.get(position));
