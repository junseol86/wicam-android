 ...
 */
public class AdvertiseAddModifyActivity extends ImageUploadingActivity {

    private Button schoolBtn, kindBtn, urlTest, submitBtn;
    private Button imageLoadButton, imageClearButton, imageRestoreButton;
    private String[] kinds = {"소식", "리크루팅", "공모전", "진로", "공연", "구인", "기타"};
    private String schoolNames, advertiseName, kind="", urlLink, description, phone1, phone2, myNickname;
    private ArrayList<String> schoolIdList, schoolIdListOriginal; // 학교를 고르다 취소할 때를 대비해, 고르기 전 카피본을 하나 만듦
    private AdvertiseData advertiseData;
    private EditText advertiseNameEditText, urlLinkEditText, descriptionEditText, phone1EditText, phone2EditText;
    private TextView advertiseNameCount, descriptionTextCount, urlLinkTextCount, phone1TextCount, phone2TextCount;
    private int imageState = 0; //Submit시 취해야 할 이미지 관련 작업 - 0:변화없음 1:이미지삽입,수정 2:이미지(있으면)삭제
    private String advertiseIdToReturn; // 입력, 수정, 삭제 후 원래 리스트로 돌아갈때 줄 itemId
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.d_advertise_add_new);
        ...
        if (addOrModify == 0) { // 추가
            ...
            schoolNames = "";
            this.advertiseData = Singleton.create().getAdvertiseData();
            for (String schoolName : advertiseData.getSchoolNameList()) {
                schoolNames += schoolName;
                if (advertiseData.getSchoolNameList().indexOf(schoolName) < advertiseData.getSchoolNameList().size() - 1) {
                    schoolNames += ", ";
                }
            }

            this.schoolIdList = (ArrayList<String>)advertiseData.getSchoolIdList().clone();
            Singleton.create().setSchoolIdList(schoolIdList);
            schoolIdListOriginal = (ArrayList<String>)schoolIdList.clone();

            advertiseName = advertiseData.getItemName();
            kind = advertiseData.getKind();
            urlLink = advertiseData.getUrlLink();
            description = advertiseData.getDescription();
            phone1 = advertiseData.getPhone1();
            phone2 = advertiseData.getPhone2();
        }
            ...
            public boolean onTouch(View v, MotionEvent event) {
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        Singleton.create().setSchoolIdList(schoolIdList);
                        startActivityForResult(new Intent(AdvertiseAddModifyActivity.this, SelectSchoolsActivity.class), 0);
                        break;
                }
                return false;
            }
        ...
        advertiseNameEditText = (EditText)findViewById(R.id.advertise_name_edit_text);
        advertiseNameCount = (TextView)findViewById(R.id.advertise_name_text_count);
        new EditTextAndTextCount(advertiseNameEditText, advertiseNameCount, 30).setEditTextAndTextCount();
        ...
        kindBtn.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        kindBtn.setTextColor(new WicamColors().TEXT_NORMAL);
                        new AlertDialog.Builder(AdvertiseAddModifyActivity.this).setTitle("종류를 선택하세요")
        ...
        urlTest.setOnClickListener(new View.OnClickListener() {
                                       ...
                                       public void onClick(View v) {
                                           if (urlLinkEditText.getText().toString().trim().equalsIgnoreCase(""))
                                               return;
                                           startActivity(new Intent(AdvertiseAddModifyActivity.this, WebViewPage.class).putExtra("url_link", getUrl(urlLinkEditText.getText().toString())));
                                       }
                                   });
        ...
        submitBtn = (Button)findViewById(R.id.advertise_add_submit);
            ...
            public boolean onTouch(View v, MotionEvent event) {
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        new AlertDialog.Builder(AdvertiseAddModifyActivity.this).setTitle("올릴 이미지")
            ...
            public boolean onTouch(View v, MotionEvent event) {
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        imageState = 2;
                        imageRestoreButton.setVisibility((addOrModify == 1 && advertiseData.getImage() != null) ? View.VISIBLE : View.GONE);
        ...
        imageRestoreButton.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        imageRestoreButton.setTextColor(new WicamColors().TEXT_NORMAL);
                        imageToUpload.setImageBitmap(advertiseData.getImage());
                        imageState = 0;
                        imageRestoreButton.setVisibility(View.GONE);
                        imageClearButton.setVisibility((addOrModify == 1 && advertiseData.getImage() != null) ? View.VISIBLE : View.GONE);
                        break;
                }
                return false;
            }
    ...
    public void setTextsByValues() { // 입력받은, 혹은 수정하기 위해 전달받은 값들에 따라 화면의 텍스트들을 세팅
        ...
        if (addOrModify == 1 && setEditTextsOnce) {
            ...
            advertiseNameEditText.setText(!advertiseNameEditText.getText().toString().equalsIgnoreCase("") ? "" : advertiseName);
            ...
            if (advertiseData.getImage() != null)
                imageToUpload.setImageBitmap(advertiseData.getImage());
    ...
    public void addAdvertiseSubmit() {
        try {
            advertiseName = new UTFConvert().convert(advertiseNameEditText.getText().toString());
        ...
        if (advertiseName.trim().equalsIgnoreCase("") || kind.trim().equalsIgnoreCase("")) {
        ...
        progressDialog.show();
        if (addOrModify == 0) { // 추가
            new AdvertiseAddModifyAsyncTask(this).execute(new Security().WEB_ADDRESS + "add_advertise.php?user_id=" + new MyCache(this).getMyId() + "&device_id=" + new MyCache(this).getDeviceId()
                    + "&user_nickname=" + myNickname + "&advertise_name=" + advertiseName + "&kind=" + kind
                    + "&description=" + description + "&url_link=" + urlLink
                    + "&phone1=" + phone1 + "&phone2=" + phone2 + "&school_id=" + schoolId);
        }
        else { // 변경
            new AdvertiseAddModifyAsyncTask(this).execute(new Security().WEB_ADDRESS + "modify_advertise.php?user_id=" + new MyCache(this).getMyId() + "&device_id=" + new MyCache(this).getDeviceId()
                    + "&default_code=" + new MyCache(this).getDefaultCode() + "&content_id=" + new MyCache(this).getContentId() + "&item_id=" + advertiseData.getItemId()
                    + "&user_nickname=" + myNickname + "&advertise_name=" + advertiseName + "&kind=" + kind
                    + "&description=" + description + "&url_link=" + urlLink
                    + "&phone1=" + phone1 + "&phone2=" + phone2 + "&school_id=" + schoolId);
        }
    ...
    public void addAdvertiseResult(final String advertiseId) {
        this.advertiseIdToReturn = advertiseId;
        if (imageState == 0) {
            progressDialog.dismiss();
            Intent intent = new Intent().putExtra("itemId", advertiseIdToReturn);
            setResult(1, intent);
            finish();
        }
            ...
            new Thread(new Runnable() {
                public void run() {
                    loadAndUploadImage.doPhotoUpload("item_" + advertiseId + ".jpg");
                }
            }).start();
        }
            ...
            new ImageDeleteAsyncTask(this).execute(new Security().WEB_ADDRESS + "image_delete.php?default_code=" + new MyCache(this).getDefaultCode()
                    ...
                    + "&image_name=" + "item_" + advertiseId + ".jpg");
        }
    }
